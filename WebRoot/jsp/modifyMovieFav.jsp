				...
				<legend>${actionname }信息</legend>
				  <table style="width:95%"  class="table table-striped table-hover table-bordered">
				  	<tr>
					    ...
					    <td width="84%">
					    	<select name="movieId" id="MovieFav_movie">
				  	...
				  	<tr>
					    ...
					    <td width="84%">
					    	<select name="userId" id="MovieFav_user">
								<c:forEach items="${list }" var="item">
									<option value="${item.id }">${item.name }</option>
								</c:forEach>
							</select>
				    ...
				    <tr>
					    <td nowrap align="right" width="15%">收藏日期:</td>
					    <td width="84%">
...
$("#MovieFav_user").val("${modifybean.user.id}");
$("#MovieFav_movie").val("${modifybean.movie.id}");
