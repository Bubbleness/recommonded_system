				...
				<legend>${actionname }信息</legend>
				  <table style="width:95%"  class="table table-striped table-hover table-bordered">
				  	<tr>
					    ...
					    <td width="84%">
					    	<select name="movieId" id="MovieComment_movie">
				  	...
				  	<tr>
					    ...
					    <td width="84%">
					    	<select name="userId" id="MovieComment_user">
								<c:forEach items="${list }" var="item">
									<option value="${item.id }">${item.name }</option>
								</c:forEach>
							</select>
				    ...
				    <tr>
					    <td nowrap align="right" width="15%">评论日期:</td>
					    <td width="84%">
				  	...
				  	<tr>
					    <td align="right" width="15%">评论内容:</td>
					    <td width="84%">
					    	<input name="content" value="${modifybean.content}" class="required" type="text" size="80"/>
					    </td>
				    </tr>
...
$("#MovieComment_user").val("${modifybean.user.id}");
$("#MovieComment_movie").val("${modifybean.movie.id}");
