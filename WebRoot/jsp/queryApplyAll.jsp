...
<body>
<form name="queryForm" id="queryForm" method="post" action="${ctx}/sys/queryApplyAll.do">
...
<%--                     		<a href="${ctx}/sys/get${actionclass}.do?uid=${item.id}">编辑</a> |  --%>
								...
								<a href="javascript:;" onclick="shenhe('同意','${item.id}')">同意</a> | 
								<a href="javascript:;" onclick="shenhe('拒绝','${item.id}')">拒绝</a> | 
								</c:if>
...
function shenhe(status,uid){
	if(window.confirm("确定要"+status+"吗?")){
		window.location.href="${ctx}/sys/checkApply.do?uid="+uid+"&status="+status;
	}
}
function toAdd(){
