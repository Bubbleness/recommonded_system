                  ...
                  <c:forEach items="${SESSION_PAGE.list}" var="item">
	                  <tr>
					    <td><input type="checkbox" name="delid" value="${item.id }"/></td>
	                    	...
	                    	<td>
	                    	<c:if test="${item.locked=='黑名单'}">
	                    		<a href="${ctx}/sys/lockuser.do?uid=${item.id}&locked=正常">恢复 | </a>
	                    	</c:if>
	                    	<c:if test="${item.locked!='黑名单'}">
	                    		<a href="${ctx}/sys/lockuser.do?uid=${item.id}&locked=黑名单">锁定 | </a>
	                    	</c:if>
